 ...
 *      They hence need a GPA of 3.5 for the remaining 220 - 29 = 191 credits to achieve a target GPA of 3.14.
 *
 * @hint
...
function toGetTargetGPA(moduleCredits, moduleGrades, totalCredits, targetGPA) {
    creditsTaken = 0
    sum = 0
    for (i=0; i<moduleCredits.length; i++) {
        creditsTaken += moduleCredits[i]
        sum += moduleCredits[i] * moduleGrades[i]
    }
    result = (targetGPA*totalCredits-sum)/(totalCredits-creditsTaken)
    roundedup = Math.ceil(result*2)
    return roundedup/2
}
